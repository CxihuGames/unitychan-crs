...
public class VMDLoaderScript {
	...
	private VMDFormat Read() {
		...
		try {
			...
			format_.morph_list = ReadMorphList(); read_count++;
			...
			if (read_count <= 2 || format_.morph_list.morph_count <= 0)
				format_.morph_list = null;
			// if (read_count <= 3 || format_.skin_list.skin_count <= 0)
	...
	private VMDFormat.MotionList ReadMotionList() {
		...
		for (int i = 0; i < result.motion_count; i++)
		{
			if (!result.motion.ContainsKey(buf[i].bone_name))
			{
	...
	private VMDFormat.SkinList ReadSkinList() {
		...
		for (int i = 0; i < result.skin_count; i++)
		{
			if (!result.skin.ContainsKey(buf[i].skin_name))
			{
	...
	private VMDFormat.VMDMorphList ReadMorphList() {
		VMDFormat.VMDMorphList result = new VMDFormat.VMDMorphList();
		result.morph_count = binary_reader_.ReadUInt32();
		result.morph = new Dictionary<string, List<VMDFormat.VMDMorphData>>();
		
		// 一度バッファに貯めてソートする
		VMDFormat.VMDMorphData[] buf = new VMDFormat.VMDMorphData[result.morph_count];
		for (int i = 0; i < result.morph_count; i++) {
			buf[i] = ReadMorphData();
		}
		Array.Sort(buf, (x,y)=>(int)x.frame_no - (int)y.frame_no);
		...
		for (int i = 0; i < result.morph_count; i++)
		{
			if (!result.morph.ContainsKey(buf[i].name))
			{
				result.morph.Add(buf[i].name, new List<VMDFormat.VMDMorphData>());
			}
			result.morph[buf[i].name].Add(buf[i]);
		}

		return result;
	}
	
	private VMDFormat.VMDMorphData ReadMorphData() {
		VMDFormat.VMDMorphData result = new VMDFormat.VMDMorphData();
		result.name = ConvertByteToString(binary_reader_.ReadBytes(15), "");
		result.frame_no = binary_reader_.ReadUInt32();
		result.weight = binary_reader_.ReadSingle();
		return result;
	}
